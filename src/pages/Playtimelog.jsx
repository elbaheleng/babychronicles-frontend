...
import { deletePlaytimeApi, getPlaytimesApi } from '../services/allApis'
...
function Playtimelog() {
 const { baby, setBaby } = useContext(babyContext)
  //console.log(baby);
  const [allPlaytimeDetails, setAllPlaytimeDetails] = useState([])
  const [deletePlaytimeStatus, setDeletePlaytimeStatus] = useState({})
  ...
  const getAllPlaytimes = async (id) => {
 
    const result = await getPlaytimesApi(id)
    if (result.status == 200) {
      setAllPlaytimeDetails(result.data)
    } else {
      toast.error("Something went wrong")
    }
  }
  // console.log(allBreastfeedingDetails);
  const deletePlaytime = async (id) => {
    
    const result = await deletePlaytimeApi(id)
    if (result.status == 200) {
      setDeletePlaytimeStatus(result.data)
    } else {
      toast.error("Something went wrong")
    }
  ...
  useEffect(() => {
    ...
    //console.log(baby1);
    setBaby(baby1)
    getAllPlaytimes(baby1._id)

  }, [ deletePlaytimeStatus])
  return (
    <>
      <Header />
      ...
      <div id='content-id' >
        <h1 className='text-3xl mb-5 text-center'>{baby?.name}'s Play time Log</h1>
        <div className='flex justify-center my-5'>
        {allPlaytimeDetails?.length > 0 ?
          <div className="overflow-x-auto mx-2">
            <table className=" border text-center" style={{borderColor: 'gray'}}>
              ...
              <tbody>
                {allPlaytimeDetails?.map((item, index) => (
                  <tr className="hover:bg-gray-50" key={index}>
                    ...
                    <td className="px-4 py-2 border">
                      <button type='button' onClick={() => deletePlaytime(item?._id)} className=" px-2 py-1 rounded text-xs" style={{background:"red",color:"white"}}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
