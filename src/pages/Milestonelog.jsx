...
import { deleteMilestoneApi, getMilestonesApi } from '../services/allApis'
...
function Milestonelog() {
 const { baby, setBaby } = useContext(babyContext)
  //console.log(baby);
  const [allMilestoneDetails, setAllMilestoneDetails] = useState([])
  const [deleteMilestoneStatus, setDeleteMilestoneStatus] = useState({})
        const options = {
       // default is `save`
       method: 'open',
  ...
  const getAllMilestones = async (id) => {
 
    const result = await getMilestonesApi(id)
    if (result.status == 200) {
      setAllMilestoneDetails(result.data)
    } else {
  ...
  const deleteMilestone = async (id) => {
    ...
    const result = await deleteMilestoneApi(id)
    if (result.status == 200) {
      setDeleteMilestoneStatus(result.data)
    } else {
      toast.error("Something went wrong")
    }
  ...
  return (
    ...
    <Header />
      ...
      <div id='content-id' >
        <h1 className='text-3xl mb-5 text-center'>{baby?.name}'s Milestones </h1>
        <div className='flex justify-center my-5'>
        {allMilestoneDetails?.length > 0 ?
          <div className="overflow-x-auto mx-2">
            <table className=" border text-center" style={{borderColor: 'gray'}}>
              ...
              <tbody>
                {allMilestoneDetails?.map((item, index) => (
                  <tr className="hover:bg-gray-50" key={index}>
                    <td className="px-4 py-2 border">{index + 1}</td>
                    <td className="px-4 py-2 border">{dayjs(item?.date).format('DD/MM/YYYY')}</td>
                    <td className="px-4 py-2 border">{item?.description}</td>
                    <td className="px-4 py-2 border">
                      <button type='button' onClick={() => deleteMilestone(item?._id)} className=" px-2 py-1 rounded text-xs" style={{background:"red",color:"white"}}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
