...
import { deleteSleepApi, getSleepsApi } from '../services/allApis'
...
function Sleeplog() {
  ...
  //console.log(baby);
  const [allSleepDetails, setAllSleepDetails] = useState([])
  const [deleteSleepStatus, setDeleteSleepStatus] = useState({})
  ...
  const getAllSleeps = async (id) => {
 
    const result = await getSleepsApi(id)
    if (result.status == 200) {
      setAllSleepDetails(result.data)
    } else {
  ...
  const deleteSleep = async (id) => {
    ...
    const result = await deleteSleepApi(id)
    if (result.status == 200) {
      setDeleteSleepStatus(result.data)
    } else {
      toast.error("Something went wrong")
    }
  ...
  return (
    <>
      ...
      <div id='content-id' >
        <h1 className='text-3xl mb-5 text-center'>{baby?.name}'s Sleep Log</h1>
        <div className='flex justify-center my-5'>
        {allSleepDetails?.length > 0 ?
          <div className="overflow-x-auto mx-2">
            <table className=" border  text-center" style={{borderColor: 'gray'}}>
              ...
              <tbody>
                {allSleepDetails?.map((item, index) => (
                  <tr className="hover:bg-gray-50" key={index}>
                    ...
                    <td className="px-4 py-2 border">
                      <button type='button' onClick={() => deleteSleep(item?._id)} className=" px-2 py-1 rounded text-xs"  style={{background:"red",color:"white"}}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
