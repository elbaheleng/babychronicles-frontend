...
import { deleteHealthApi, getHealthsApi } from '../services/allApis'
...
function Healthlog() {
  ...
  //console.log(baby);
  const [allHealthDetails, setAllHealthDetails] = useState([])
  const [deleteHealthStatus, setDeleteHealthStatus] = useState({})
  ...
  const getAllHealths = async (id) => {

    const result = await getHealthsApi(id)
    if (result.status == 200) {
      setAllHealthDetails(result.data)
    } else {
  ...
  const deleteHealth = async (id) => {
    ...
    const result = await deleteHealthApi(id)
    if (result.status == 200) {
      setDeleteHealthStatus(result.data)
    } else {
      toast.error("Something went wrong")
    }
  ...
  return (
    <>
      ...
      <div id='content-id' >
        <h1 className='text-3xl mb-5 text-center'>{baby?.name}'s Health/Vaccine Log</h1>
        <div className='flex justify-center my-5'>
        {allHealthDetails?.length > 0 ?
          <div className="overflow-x-auto mx-2">
            <table className=" border text-center" style={{borderColor: 'gray'}}>
              ...
              <tbody>
                {allHealthDetails?.map((item, index) => (
                  <tr className="hover:bg-gray-50" key={index}>
                    ...
                    <td className="px-4 py-2 border">
                      <button type='button' onClick={() => deleteHealth(item?._id)} className=" px-2 py-1 rounded text-xs" style={{background:"red",color:"white"}}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <p>No Health/Vaccine log added yet.</p>}
      </div>
      </div>
    </>
  )
